...
watch(y, async (newValue) => {
    if (
        newValue + window.innerHeight === document.body.scrollHeight && 
    ...
    <div>
        ...
        <div class="mt-4 lg:mt-6">
            <ClientOnly>
                ...
                <div 
                    v-if="display === 'grid'" 
                    class="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-3 place-items-center"
                >
                    <GridItemCard 
                        v-for="item in items" 
                        :key="item.id" 
